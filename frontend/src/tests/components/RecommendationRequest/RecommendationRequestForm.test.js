...
describe("RecommendationRequestForm tests", () => {
    ...
    test("No Error messsages on good input", async () => {
        ...
        fireEvent.change(professorEmailField, { target: { value: 'abc@ucsb.edu' } });
        fireEvent.change(explanationField, { target: { value: 'BS/MS Program' } });
        fireEvent.change(dateRequestedField, { target: { value: '2022-01-02T12:00:10' } });
        fireEvent.change(dateNeededField, { target: { value: '2023-01-02T12:00:12' } });
        fireEvent.change(doneField, { target: { value: 'false' } })
        fireEvent.click(submitButton);

        await waitFor(() => expect(mockSubmitAction).toHaveBeenCalled());

        expect(screen.queryByText(/RequesterEmail must be an email/)).not.toBeInTheDocument();
        expect(screen.queryByText(/ProfessorEmail must be an email/)).not.toBeInTheDocument();
